...
export default async function Page({ searchParams }) {
  ...
  let selectedMonth = searchParams?.month;

  if (!mode || !selectedMonth) {

    const currentPeriod = getCurrentPayrollPeriod(new Date());

    const start = currentPeriod.startDate;

    selectedMonth = `${start.getFullYear()}-${String(
      start.getMonth() + 1
    ).padStart(2, "0")}`;

    mode =
      currentPeriod.round === "10–25"
        ? "10to25"
        : "26to09";
  }
  ...
  const baseRows = mapUsersToDailyRow(
    users,
    checkins,
    dates,
    leaves,
